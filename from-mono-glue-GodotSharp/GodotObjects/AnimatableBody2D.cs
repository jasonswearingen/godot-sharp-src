...
/// <para>When <see cref="Godot.AnimatableBody2D"/> is moved, its linear and angular velocity are estimated and used to affect other physics bodies in its path. This makes it useful for moving platforms, doors, and other moving objects.</para>
/// </summary>
public partial class AnimatableBody2D : StaticBody2D
{
    /// <summary>
    /// <para>If <see langword="true"/>, the body's movement will be synchronized to the physics frame. This is useful when animating movement via <see cref="Godot.AnimationPlayer"/>, for example on moving platforms. Do <b>not</b> use together with <see cref="Godot.PhysicsBody2D.MoveAndCollide(Vector2, bool, float, bool)"/>.</para>
    /// </summary>
    ...
    private static readonly System.Type CachedType = typeof(AnimatableBody2D);

    private static readonly StringName NativeName = "AnimatableBody2D";

    [DebuggerBrowsable(DebuggerBrowsableState.Never)]
    private static readonly unsafe delegate* unmanaged<IntPtr> NativeCtor = ClassDB_get_constructor(NativeName);

    public AnimatableBody2D() : this(false)
    {
        unsafe
        {
    ...
    internal AnimatableBody2D(bool memoryOwn) : base(memoryOwn) { }
